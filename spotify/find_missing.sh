...
find_missing(){
    ...
    while read line; do
        if [ $nolookup -eq 0 ]; then
            ...
            track_name="$(perl -pne 's/^The //i; s/ - \(?(?:\d{2,4}\s+)?(?:(?:UK )?Radio|(?:Digital )?Re-?master(?:ed)?|Single|Album|Amended|Main|Uncut|Edit|Explicit|Clean|Mix|Original|Re-edit|Bonus Track|'"'"'?\w+ Version)(\s.*)?$//i' <<< "$track_name")"
