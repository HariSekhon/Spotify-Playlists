...
find_missing(){
    ...
    while read line; do
        if [ $nolookup -eq 0 ]; then
            ...
            s/ (?:- \(?|\()(?:(?:\d{1,4}"|New|US|UK)\s+)?(?:Radio|(?:Digital )?Re-?master(?:ed)?|Single|Album|Amended|Main|Uncut|(?:Mainstream )?Edit|Explicit|Clean|Mix|Original|Re-edit|Bonus Track|'"'"'?\w+ Version|(?:as )?made famous|theme from|from)([\s\)].*)?$//i;
