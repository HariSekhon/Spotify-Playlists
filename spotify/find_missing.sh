...
find_missing(){
    ...
    while read line; do
        if [ $nolookup -eq 0 ]; then
            ...
            track_name="$(perl -pne 's/^The //i; s/ - \(?(?:\d{2,4}\s+)?(?:(?:Original )?Radio|(?:Digital )?Re-?master(?:ed)?|Single|Album|Amended|Main|Uncut|Edit|Explicit|Mix|Original (?:Mix|Version)|UK Radio|Clean|Re-edit|Bonus Track|'"'"'?\w+ Version)(\s.*)?$//i' <<< "$track_name")"
